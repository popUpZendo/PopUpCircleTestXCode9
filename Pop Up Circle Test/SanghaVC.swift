...
extension SanghaVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        DataService.instance.getAllBodhi { (returnedBodhi) in
            cell.configureSanghaCell(profile_image: image!, name: bodhi.name, popUpGroup: bodhi.popUpGroup, city: bodhi.city, state: bodhi.state, temple: bodhi.temple, teacher: bodhi.teacher, practice: bodhi.practice, senderId: bodhi.senderId)
        }
        return cell
    }
}
